...
- **`wordpress`** — `wordpress:php8.2-fpm`. PHP-FPM only, on port 9000. The official image
  **auto-generates `wordpress/wp-config.php`** from the `WORDPRESS_*` env vars on first boot — no
  ...
  Upload/memory limits (`upload_max_filesize`, `post_max_size`, `memory_limit`) come from
  `./php/uploads.ini`, mounted read-only into `/usr/local/etc/php/conf.d/` — edit it and
  `docker compose restart wordpress` to change them.
...
| `./scripts/scan-wp-files.sh [dir]` | Verify the WordPress files (defaults to `./wordpress`). **Layer 0** is a fatal completeness gate — if core is missing/incomplete it reports exactly what's missing and exits non-zero. If that passes, it reports (never deletes): files that don't belong to a standard install (filesystem heuristics) and, via `wp core verify-checksums`, core files that have been modified. The full output is also saved to `wp-scan-report.txt` (gitignored; override with `WP_SCAN_REPORT`). |
| `./scripts/start.sh` | Bring the stack up — frees the host ports it needs (80/443/…) first, offering to stop whatever holds them. Then runs `scan-wp-files.sh` to verify the provided `./wordpress` in one step: if core is incomplete it reports what's missing and `docker compose down`s the stack (check only — never downloads or modifies WordPress); otherwise it reports any cruft / modified core files and continues. Run interactively it then offers a **setup wizard**: fix `wp-config.php` creds → import a DB dump → rewrite the site domain. (`--no-wizard` to skip.) |
