...
| `./scripts/start.sh` | Bring the stack up — frees the host ports it needs (80/443/…) first, offering to stop whatever holds them. If an imported `./wordpress` is missing WordPress core (the image skips its copy when files already exist), detects it and offers `wp core download`. Run interactively it then offers a **setup wizard**: fix `wp-config.php` creds → import a DB dump → rewrite the site domain. (`--no-wizard` to skip.) |
